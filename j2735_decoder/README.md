...
### 5. **Test Your Setup**:
   ...
   For an initial test, execute the `decoder.py` script. You can modify the payload values in this script as needed, and then inspect the XML and JSON outputs directly in the terminal.

   *  **NOTE**: before running _decoder.py_, run ```~/Projects/RSU_Tools/change_py_headers.sh``` fo fix the python header path
